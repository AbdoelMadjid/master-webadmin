...
if (!function_exists('userAvatarInitials')) {
    function userAvatarInitials(?string $name, ?string $email = null): string
    {
        $base = trim((string) ($name ?: $email ?: 'User'));
        if ($base === '') {
            return 'U';
        }
        ...
        if ($initials === '') {
            $initials = strtoupper(substr($base, 0, 1));
        }
...
if (!function_exists('userInitialAvatarDataUri')) {
    function userInitialAvatarDataUri(?string $name, ?string $email = null, ?string $seed = null): string
    {
        $key = (string) ($seed ?: $email ?: $name ?: 'guest');
        $bgColor = warnaDariId($key);
        $textColor = kontrasTeks($bgColor);
        $initials = userAvatarInitials($name, $email);

        $svg = '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100">'
            . '<rect width="100" height="100" fill="' . $bgColor . '"/>'
            . '<text x="50%" y="50%" dominant-baseline="central" text-anchor="middle"'
            . ' font-family="Arial, Helvetica, sans-serif" font-size="38" font-weight="700" fill="' . $textColor . '">'
...
if (!function_exists('normalizeAvatarUrl')) {
    function normalizeAvatarUrl($value): ?string
    {
        ...
        $value = trim($value);
        if ($value === '' || in_array(strtolower($value), ['null', 'undefined', 'false'], true)) {
            return null;
        }
...
if (!function_exists('getUserAvatarUrl')) {
    function getUserAvatarUrl($user = null, ?array $displayData = null): string
    {
        $user = $user ?: Auth::user();

        $candidateSources = [];

        if (is_array($displayData) && array_key_exists('avatar', $displayData)) {
            $candidateSources[] = $displayData['avatar'];
        }
        ...
        if ($user) {
            ...
            $candidateSources[] = $user->image ?? null;
        }

        foreach ($candidateSources as $source) {
            $normalized = normalizeAvatarUrl($source);
            if ($normalized !== null) {
                return $normalized;
            }
        }
