@php
    ...
    $headerTitleClass = $headerTitleClass ?? 'card-title align-items-start flex-column';
    $title = $title ?? 'Instagram Subscribers';
    $subtitle = $subtitle ?? '75% activity growth';
    $subtitleClass = $subtitleClass ?? 'text-gray-500 pt-2 fw-semibold fs-6';
    $toolbarIconClass = $toolbarIconClass ?? 'ki-duotone ki-dots-square fs-1 text-gray-500 me-n1';
    $bodyClass = $bodyClass ?? 'card-body pt-0 px-0';
    $chartId = $chartId ?? 'kt_charts_widget_1';
    $chartClass = $chartClass ?? 'min-h-auto ps-4 pe-6 mb-3';
    $chartStyle = $chartStyle ?? 'height: 350px';
...
<div class="{{ $cardClass }}">
    ...
    <div class="{{ $bodyClass }}">
        @if ($bodyView)
            @include($bodyView, array_merge(['chartId' => $chartId, 'chartClass' => $chartClass, 'chartStyle' => $chartStyle], $bodyData))
        @else
            <!--begin::Chart-->
