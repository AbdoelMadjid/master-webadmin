    ...
    <div class="card-header align-items-center py-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bold text-gray-900 fs-4">
                <i class="ki-duotone ki-time fs-2 text-primary me-2"><span class="path1"></span><span
                        class="path2"></span></i>
        ...
        <div class="timeline">
            @forelse($versions as $index => $ver)
                ...
                <div class="timeline-item">
                    ...
                    <!--begin::Timeline icon-->
                    <div
                        class="timeline-icon symbol symbol-circle symbol-40px {{ $index === 0 ? 'bg-light-success' : 'bg-light-primary' }} me-4">
                        ...
                        <div class="card bg-light-secondary border border-gray-300 p-5 rounded shadow-xs">
                            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    <span class="badge badge-light-primary fs-7 fw-bold">
                                        <i class="ki-duotone ki-time fs-7 me-1 text-primary"><span
                                    ...
                                    @if (!empty($ver['id']))
                                        <span data-bs-toggle="tooltip" data-bs-placement="top" title="Edit Versi Rilis">
                                            <button type="button"
                                                class="btn btn-icon btn-sm btn-light-warning shadow-xs h-25px w-25px"
                                                data-changelog="{{ json_encode($ver) }}"
                                                onclick="openEditChangelogFromBtn(this)">
                                                <i class="ki-duotone ki-pencil fs-6 p-0 m-0"><span
                                                        class="path1"></span><span class="path2"></span></i>
                                            </button>
                                        </span>
                                        <span data-bs-toggle="tooltip" data-bs-placement="top"
                                            ...
                                            <button type="button"
                                                class="btn btn-icon btn-sm btn-light-danger shadow-xs h-25px w-25px"
                                                onclick="deleteChangelog({{ $ver['id'] }}, '{{ $ver['version'] }}')">
                                                <i class="ki-duotone ki-trash fs-6 p-0 m-0"><span
                            ...
                            <div class="mb-4">
                                <h5 class="fs-7 text-uppercase fw-bold text-gray-500 mb-2">
                                    <i class="ki-duotone ki-check-square fs-6 text-primary me-1"><span
                                            class="path1"></span><span class="path2"></span></i>
                                ...
                                <div class="d-flex flex-column gap-2">
                                    @foreach ($ver['highlights'] as $hl)
                                        <div class="d-flex align-items-start gap-2">
                                            @if ($hl['type'] === 'feat')
                                                <span
                                                    class="badge badge-light-success fs-8 fw-bold min-w-75px text-center">FEAT</span>
                                            @elseif($hl['type'] === 'fix')
                                                <span
                                                    class="badge badge-light-danger fs-8 fw-bold min-w-75px text-center">FIX</span>
                                            @elseif($hl['type'] === 'ui')
                                                <span
                                                    class="badge badge-light-info fs-8 fw-bold min-w-75px text-center">UI</span>
                                            @elseif($hl['type'] === 'docs')
                                                <span
                                                    class="badge badge-light-warning fs-8 fw-bold min-w-75px text-center">DOCS</span>
                                            @else
                                                <span
                                                    class="badge badge-light-primary fs-8 fw-bold min-w-75px text-center">REFACTOR</span>
                                            @endif
                            ...
                            <div class="accordion accordion-icon-toggle" id="kt_accordion_commits_{{ $index }}">
                                <div class="accordion-item border-0">
                                    ...
                                    <div id="kt_accordion_body_{{ $index }}" class="fs-6 collapse mt-3"
                                        data-bs-parent="#kt_accordion_commits_{{ $index }}">
                                                ...
                                                <thead>
                                                    <tr class="text-muted fw-bold text-uppercase gs-0">
                                                        ...
                                                        <th class="w-100px text-end">
                                                            {{ app()->getLocale() == 'en' ? 'Date' : 'Tanggal' }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach ($ver['commits'] as $c)
                                                        <tr>
                                                            <td><code
                                                                    class="text-primary bg-light-primary px-2 py-1 rounded">{{ $c['hash'] }}</code>
                                                            </td>
            ...
            @empty
                <div class="text-center py-10 text-muted">
                    <i class="ki-duotone ki-information-5 fs-3x mb-3"><span class="path1"></span><span
                            class="path2"></span><span class="path3"></span></i>
                    <div>
                        {{ app()->getLocale() == 'en' ? 'No version history found.' : 'Tidak ada riwayat versi ditemukan.' }}
                    </div>
                </div>
            @endforelse
        </div>
