...
<div class="card card-flush">
    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        ...
        <div class="card-title d-flex flex-wrap align-items-center gap-3">
            ...
            <div class="w-250px">
                <select class="form-select form-select-solid" id="filter_kategori_select"
                    onchange="filterItemByKategori(this.value)">
                    <option value="">
                        {{ app()->getLocale() == 'en' ? '-- All Categories --' : '-- Semua Kategori --' }}</option>
                    @foreach ($kategoris as $kat)
                        <option value="{{ $kat->id }}"
                            {{ (string) $selectedKategoriId === (string) $kat->id ? 'selected' : '' }}>
                            {{ $kat->nama }} ({{ $kat->kode }})
                        </option>
                    @endforeach
                </select>
            ...
            <div class="d-flex align-items-center position-relative">
                ...
                <input type="text" id="kt_referensi_item_search"
                    value="{{ $activeTab === 'item' ? $searchQuery : '' }}"
        ...
        <div class="card-toolbar d-flex align-items-center gap-2 ms-auto">
            <span data-bs-toggle="tooltip" data-bs-placement="top"
                title="{{ app()->getLocale() == 'en' ? 'Add New Reference Choice Option' : 'Tambah Opsi Pilihan Item Referensi Baru' }}">
                <button type="button"
                    ...
                    data-bs-toggle="modal" data-bs-target="#kt_modal_referensi_item" onclick="resetItemForm()">
                    <i class="ki-duotone ki-element-plus fs-2 p-0 m-0"><span class="path1"></span><span
                            class="path2"></span><span class="path3"></span><span class="path4"></span><span
                            class="path5"></span></i>
                    <span
                        class="d-none d-sm-inline ms-2">{{ app()->getLocale() == 'en' ? 'Add Item' : 'Tambah Item' }}</span>
                </button>
            </span>
        </div>
        <!--end::Card toolbar-->
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5 w-100" id="kt_table_referensi_item">
                <thead>
                    <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                        ...
                        <th class="min-w-150px">{{ app()->getLocale() == 'en' ? 'Option Label' : 'Nama Label Opsi' }}
                        </th>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge badge-light-primary fw-bold fs-7">
                                    ...
                                    <code
                                        class="text-muted ms-1">({{ $itm->kategori ? $itm->kategori->kode : '-' }})</code>
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                <div
                                    ...
                                    <input class="form-check-input h-20px w-40px cursor-pointer" type="checkbox"
                                        id="item_switch_{{ $itm->id }}" {{ $itm->is_active ? 'checked' : '' }}
                                        onchange="toggleItemStatus({{ $itm->id }}, this)" data-bs-toggle="tooltip"
                                        title="{{ app()->getLocale() == 'en' ? 'Toggle item status' : 'Ubah status aktif item' }}" />
                                </div>
                            </td>
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-2">
                                    <span data-bs-toggle="tooltip" data-bs-placement="top"
                                        title="{{ app()->getLocale() == 'en' ? 'Edit Item' : 'Edit Item' }}">
                                        <button type="button" class="btn btn-icon btn-sm btn-light-warning"
                                            onclick="editItem({{ json_encode($itm) }})">
                                            <i class="ki-duotone ki-pencil fs-4"><span class="path1"></span><span
                                                    class="path2"></span></i>
                                        </button>
                                    </span>
                                    <span data-bs-toggle="tooltip" data-bs-placement="top"
                                        title="{{ app()->getLocale() == 'en' ? 'Delete Item' : 'Hapus Item' }}">
                                        <button type="button" class="btn btn-icon btn-sm btn-light-danger"
                                            onclick="deleteItem({{ $itm->id }}, '{{ addslashes($itm->nama) }}')">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-10 text-muted">
                                ...
                                <div>
                                    {{ app()->getLocale() == 'en' ? 'No reference items found for selected filter.' : 'Tidak ada item referensi yang ditemukan.' }}
                                </div>
                            </td>
                        </tr>
                    @endforelse
