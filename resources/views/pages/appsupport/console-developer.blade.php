        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-6 bg-white p-5 rounded border border-gray-200 shadow-xs">
            ...
            <div class="d-flex align-items-center gap-2 ms-auto">
                <span data-bs-toggle="tooltip" data-bs-placement="top" title="{{ app()->getLocale() == 'en' ? 'Operational Guide' : 'Petunjuk Operasional' }}">
                    <button type="button" class="btn btn-icon btn-danger shadow-xs d-inline-flex align-items-center justify-content-center w-35px h-35px p-0" data-bs-toggle="modal" data-bs-target="#kt_modal_console_developer_help">
        ...
        <div class="card card-flush shadow-sm mb-6">
            <div class="card-body pt-4 pb-0">
                <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bold">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-active-primary pb-4 {{ $activeTab === 'git-manager' ? 'active' : '' }}" href="{{ route('appsupport.console-developer', ['tab' => 'git-manager']) }}">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-active-primary pb-4 {{ $activeTab === 'setup-maintenance' ? 'active' : '' }}" href="{{ route('appsupport.console-developer', ['tab' => 'setup-maintenance']) }}">
                            <i class="ki-duotone ki-wrench fs-2 me-2"><span class="path1"></span><span class="path2"></span></i>
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-active-primary pb-4 {{ $activeTab === 'crud-generator' ? 'active' : '' }}" href="{{ route('appsupport.console-developer', ['tab' => 'crud-generator']) }}">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-active-primary pb-4 {{ $activeTab === 'file-utilities' ? 'active' : '' }}" href="{{ route('appsupport.console-developer', ['tab' => 'file-utilities']) }}">
...
<script>
    ...
    function triggerGitAction(action, params = {}) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: Object.assign({ request_type: 'git', action: action }, params),
    ...
    function submitGitCommit(e) {
        ...
        triggerGitAction('commit_push', { commit_message: msg });
    ...
    function promptCreateTag() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                triggerGitAction('create_tag', { tag_name: result.value });
            }
        });
    }
    ...
    function promptForceTag() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                triggerGitAction('force_tag', { tag_name: result.value });
            }
        });
    }
    ...
    function promptDeleteTag() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                triggerGitAction('delete_tag', { tag_name: result.value });
            }
        });
    }
    ...
    function promptSwitchBranch() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                triggerGitAction('switch_branch', { branch_name: result.value });
            }
        });
    }
    ...
    function promptAutoRelease() {
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                if (!tag) {
                    Swal.showValidationMessage("{{ app()->getLocale() == 'en' ? 'Tag version is required!' : 'Versi tag wajib diisi!' }}");
                    return false;
                }
                return {
                    commit_message: document.getElementById('swal_auto_commit').value || 'Auto release update',
                    tag_name: tag
                };
            }
        }).then((result) => {
    ...
    function triggerMaintenance(action) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: { request_type: 'maintenance', action: action },
            success: function(res) {
                Swal.close();
                if (res.success) {
