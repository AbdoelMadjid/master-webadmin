...
<form id="form_profil_pengaturan" action="{{ route('profil-pengguna.pengaturan.update') }}" method="POST" class="form">
    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
            data-bs-target="#kt_account_identitas_ktp">
            ...
            <div class="card-body border-top p-9">
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-6 fv-row">
                        ...
                        <input type="text" name="nama_lengkap"
                            ...
                            value="{{ old('nama_lengkap', $detail?->nama_lengkap ?? $user?->name) }}" required />
                    </div>
                </div>
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row mb-4 mb-lg-0">
                        ...
                        <input type="text" name="tempat_lahir"
                            class="form-control form-control-lg form-control-solid" placeholder="Contoh: Jakarta"
                    ...
                    <div class="col-lg-6 fv-row">
                        ...
                        <input type="date" name="tanggal_lahir"
                            class="form-control form-control-lg form-control-solid"
                            value="{{ old('tanggal_lahir', $detail?->tanggal_lahir ? $detail->tanggal_lahir->format('Y-m-d') : '') }}" />
                    </div>
                </div>
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row mb-4 mb-lg-0">
                        ...
                        <select name="jenis_kelamin" class="form-select form-select-solid form-select-lg fw-semibold"
                            ...
                            @if (isset($referensis['JENKEL']) && $referensis['JENKEL']->activeItems->count() > 0)
                                @foreach ($referensis['JENKEL']->activeItems as $item)
                                    <option value="{{ $item->kode }}"
                                        {{ old('jenis_kelamin', $detail?->jenis_kelamin) == $item->kode ? 'selected' : '' }}>
                                        {{ $item->nama }}</option>
                                @endforeach
                            @else
                                <option value="L"
                                    {{ old('jenis_kelamin', $detail?->jenis_kelamin) == 'L' ? 'selected' : '' }}>
                                    Laki-laki</option>
                                <option value="P"
                                    {{ old('jenis_kelamin', $detail?->jenis_kelamin) == 'P' ? 'selected' : '' }}>
                                    Perempuan</option>
                            @endif
                        </select>
                    </div>
                    <div class="col-lg-6 fv-row">
                        ...
                        <select name="golongan_darah" class="form-select form-select-solid form-select-lg fw-semibold"
                            data-control="select2" data-placeholder="Pilih Golongan Darah">
                            ...
                            @if (isset($referensis['GOLONGAN_DARAH']) && $referensis['GOLONGAN_DARAH']->activeItems->count() > 0)
                                @foreach ($referensis['GOLONGAN_DARAH']->activeItems as $item)
                                    <option value="{{ $item->kode }}"
                                        {{ old('golongan_darah', $detail?->golongan_darah) == $item->kode ? 'selected' : '' }}>
                                        {{ $item->nama }}</option>
                                @endforeach
                            @else
                                @foreach (['A', 'B', 'AB', 'O'] as $gol)
                                    <option value="{{ $gol }}"
                                        {{ old('golongan_darah', $detail?->golongan_darah) == $gol ? 'selected' : '' }}>
                                        Golongan {{ $gol }}</option>
                                @endforeach
                            @endif
                        </select>
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row mb-4 mb-lg-0">
                        ...
                        <select name="agama" class="form-select form-select-solid form-select-lg fw-semibold"
                            ...
                            @if (isset($referensis['AGAMA']) && $referensis['AGAMA']->activeItems->count() > 0)
                                @foreach ($referensis['AGAMA']->activeItems as $item)
                                    <option value="{{ $item->nama }}"
                                        {{ old('agama', $detail?->agama) == $item->nama || old('agama', $detail?->agama) == $item->kode ? 'selected' : '' }}>
                                        {{ $item->nama }}</option>
                                @endforeach
                            @else
                                @foreach (['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Khonghucu', 'Lainnya'] as $agm)
                                    <option value="{{ $agm }}"
                                        {{ old('agama', $detail?->agama) == $agm ? 'selected' : '' }}>
                                        {{ $agm }}</option>
                                @endforeach
                            @endif
                        </select>
                        ...
                        <select name="status_perkawinan"
                            ...
                            @if (isset($referensis['STATUS_PERKAWINAN']) && $referensis['STATUS_PERKAWINAN']->activeItems->count() > 0)
                                @foreach ($referensis['STATUS_PERKAWINAN']->activeItems as $item)
                                    <option value="{{ $item->nama }}"
                                        {{ old('status_perkawinan', $detail?->status_perkawinan) == $item->nama || old('status_perkawinan', $detail?->status_perkawinan) == $item->kode ? 'selected' : '' }}>
                                        {{ $item->nama }}</option>
                                @endforeach
                            @else
                                @foreach (['Belum Kawin', 'Kawin', 'Cerai Hidup', 'Cerai Mati'] as $status)
                                    <option value="{{ $status }}"
                                        {{ old('status_perkawinan', $detail?->status_perkawinan) == $status ? 'selected' : '' }}>
                                        {{ $status }}</option>
                                @endforeach
                            @endif
                        </select>
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row mb-4 mb-lg-0">
                        ...
                        <input type="text" name="pekerjaan" class="form-control form-control-lg form-control-solid"
                            placeholder="Contoh: Karyawan Swasta, PNS, Pengusaha"
                            value="{{ old('pekerjaan', $detail?->pekerjaan) }}" />
                    </div>
                    <div class="col-lg-6 fv-row">
                        <label class="form-label fw-semibold fs-6">Kewarganegaraan</label>
                        <select name="kewarganegaraan" class="form-select form-select-solid form-select-lg fw-semibold"
                            data-control="select2" data-placeholder="Pilih Kewarganegaraan">
                            <option value="WNI"
                                {{ old('kewarganegaraan', $detail?->kewarganegaraan ?? 'WNI') == 'WNI' ? 'selected' : '' }}>
                                Warga Negara Indonesia (WNI)</option>
                            <option value="WNA"
                                {{ old('kewarganegaraan', $detail?->kewarganegaraan) == 'WNA' ? 'selected' : '' }}>
    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
            data-bs-target="#kt_account_kontak_telepon">
            <div class="card-title m-0">
                <h3 class="fw-bold m-0 text-gray-900">
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row mb-4 mb-lg-0">
                        ...
                        <div class="input-group input-group-solid input-group-lg">
                            <span class="input-group-text"><i class="ki-duotone ki-whatsapp fs-3"><span
                                        class="path1"></span><span class="path2"></span></i></span>
                            <input type="tel" name="no_hp"
                                class="form-control form-control-lg form-control-solid"
                                placeholder="Contoh: 081234567890" value="{{ old('no_hp', $detail?->no_hp) }}" />
                        </div>
                    </div>
                    <div class="col-lg-6 fv-row">
                        <label class="form-label fw-semibold fs-6">Alamat Email (Akun)</label>
                        <div class="input-group input-group-solid input-group-lg">
                            <span class="input-group-text"><i class="ki-duotone ki-sms fs-3"><span
                                        class="path1"></span><span class="path2"></span></i></span>
                            <input type="email" class="form-control form-control-lg form-control-solid bg-secondary"
                                value="{{ $user?->email }}" readonly data-bs-toggle="tooltip"
                                data-bs-placement="top" title="Email terikat dengan akun Anda" />
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
            data-bs-target="#kt_account_alamat_lengkap">
            <div class="card-title m-0">
                <h3 class="fw-bold m-0 text-gray-900">
            ...
            <div class="card-body border-top p-9">
                ...
                <div class="row mb-6">
                    <div class="col-lg-8 fv-row mb-4 mb-lg-0">
                        ...
                        <input type="text" name="alamat_jalan"
                            class="form-control form-control-lg form-control-solid"
                            placeholder="Contoh: Jl. Merdeka / Perum Melati"
                            value="{{ old('alamat_jalan', $detail?->alamat_jalan) }}" />
                    </div>
                    <div class="col-lg-4 fv-row">
                        <label class="form-label fw-semibold fs-6">Nomor Rumah</label>
                        <input type="text" name="no_rumah" class="form-control form-control-lg form-control-solid"
                            placeholder="Contoh: No. 12B" value="{{ old('no_rumah', $detail?->no_rumah) }}" />
                    </div>
                </div>
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row mb-4 mb-lg-0">
                        ...
                        <input type="text" name="rt_rw" class="form-control form-control-lg form-control-solid"
                            placeholder="Contoh: 005/002" value="{{ old('rt_rw', $detail?->rt_rw) }}" />
                    </div>
                    <div class="col-lg-6 fv-row">
                        ...
                        <input type="text" name="blok" class="form-control form-control-lg form-control-solid"
                            placeholder="Contoh: Blok A3" value="{{ old('blok', $detail?->blok) }}" />
                    </div>
                </div>
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row mb-4 mb-lg-0">
                        ...
                        <input type="text" name="desa_kelurahan"
                            ...
                            value="{{ old('desa_kelurahan', $detail?->desa_kelurahan) }}" />
                    </div>
                    <div class="col-lg-6 fv-row">
                        <label class="form-label fw-semibold fs-6">Kecamatan</label>
                        <input type="text" name="kecamatan"
                            class="form-control form-control-lg form-control-solid"
                            placeholder="Contoh: Kec. Lengkong"
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row mb-4 mb-lg-0">
                        ...
                        <input type="text" name="kabupaten_kota"
                            class="form-control form-control-lg form-control-solid" placeholder="Contoh: Kota Bandung"
                            value="{{ old('kabupaten_kota', $detail?->kabupaten_kota) }}" />
                    </div>
                    <div class="col-lg-6 fv-row">
                        ...
                        <input type="text" name="provinsi" class="form-control form-control-lg form-control-solid"
                            placeholder="Contoh: Jawa Barat" value="{{ old('provinsi', $detail?->provinsi) }}" />
                    </div>
                </div>
                ...
                <div class="row mb-6">
                    <div class="col-lg-6 fv-row">
                        ...
                        <input type="text" name="kode_pos" class="form-control form-control-lg form-control-solid"
                            placeholder="Contoh: 40262" maxlength="10"
                            value="{{ old('kode_pos', $detail?->kode_pos) }}" />
                    </div>
                </div>
            </div>
        </div>
                    ...
                    if (response.ok && data.success) {
                        if (typeof SwalHelper !== 'undefined' && SwalHelper.success) {
                            SwalHelper.success(data.message ||
                                'Data identitas profil berhasil diperbarui!');
