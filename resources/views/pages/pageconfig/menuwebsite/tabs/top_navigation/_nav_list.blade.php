<div class="card card-flush shadow-xs border border-gray-200">
    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
        <div class="card-title">
            <form method="GET" action="{{ route('pageconfig.menuwebsite.top-navigation') }}" class="d-flex align-items-center position-relative my-1">
                <input type="hidden" name="tab" value="nav-list">
                <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-4">
                    <span class="path1"></span><span class="path2"></span>
                </i>
                <input type="text" name="q" value="{{ $searchQuery }}" class="form-control form-control-solid w-250px ps-12" placeholder="{{ app()->getLocale() == 'en' ? 'Search top navigation...' : 'Cari navigasi atas...' }}" />
            </form>
        </div>
    </div>
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-1">
                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="{{ app()->getLocale() == 'en' ? 'Edit Item' : 'Edit Navigasi' }}">
                                        <button type="button" class="btn btn-icon btn-light-primary btn-sm btn-edit-navigation" data-bs-toggle="modal" data-bs-target="#kt_modal_top_navigation" data-nav="{{ json_encode($nav) }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-10 text-muted">
                                ...
                                <div class="fs-6 fw-semibold">{{ app()->getLocale() == 'en' ? 'No top navigation items found.' : 'Tidak ada data navigasi atas ditemukan.' }}</div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
