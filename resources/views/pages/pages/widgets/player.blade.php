...
@section('content')
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl widget-showcase-masonry">
            @php
                $availableWidgets = collect(glob(resource_path('views/partials/widgets/player/__widgets-*.blade.php')))
                    ->map(function ($path) {
            ...
            <div class="row g-5 g-xl-10 widget-showcase-row">
                @foreach ($availableWidgets as $id)
                    <div class="widget-showcase-item">
                        <x-widget-include-badge name="player._widget-{{ $id }}" />
                        @include('partials.widgets.player.__widgets-' . $id)
                    </div>
                @endforeach
            </div>
        </div>
