...
@section('content')
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl widget-showcase-masonry">
            @php
                $availableWidgets = collect(glob(resource_path('views/partials/widgets/list/__widgets-*.blade.php')))
                    ->map(function ($path) {
            ...
            <div class="row g-5 g-xl-10 widget-showcase-row">
                @foreach ($orderedWidgets as $id)
                    <div class="widget-showcase-item">
                        <x-widget-include-badge name="list._widget-{{ $id }}" />
                        @include('partials.widgets.list.__widgets-' . $id)
                    </div>
                @endforeach
            </div>
        </div>
