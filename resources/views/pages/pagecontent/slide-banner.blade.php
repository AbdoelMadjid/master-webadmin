        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-6 bg-white p-5 rounded border border-gray-200 shadow-xs">
                ...
                <div class="d-flex align-items-center gap-2 ms-auto">
                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="{{ app()->getLocale() == 'en' ? 'Add Slide Banner' : 'Tambah Slide Banner' }}">
                        <button type="button" class="btn btn-primary shadow-xs d-inline-flex align-items-center justify-content-center w-35px w-sm-auto h-35px px-0 px-sm-4" onclick="openAddBannerModal()">
                            <i class="ki-duotone ki-plus fs-2 p-0 m-0"><span class="path1"></span><span class="path2"></span></i>
                            <span class="d-none d-sm-inline ms-2">{{ app()->getLocale() == 'en' ? 'Add Banner' : 'Tambah Banner' }}</span>
                        </button>
                    </span>

                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="{{ app()->getLocale() == 'en' ? 'Operational Guide' : 'Petunjuk Operasional' }}">
                        <button type="button" class="btn btn-danger shadow-xs d-inline-flex align-items-center justify-content-center w-35px h-35px p-0" data-bs-toggle="modal" data-bs-target="#kt_modal_slide_banner_help">
                        ...
                        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                            <thead>
                                <tr class="fw-bold text-muted bg-light">
                                    ...
                                    <th class="min-w-200px">{{ app()->getLocale() == 'en' ? 'Title Prefix & Highlight' : 'Judul Prefix & Highlight' }}</th>
                                    <th class="min-w-200px">{{ app()->getLocale() == 'en' ? 'Description' : 'Deskripsi' }}</th>
                                    <th class="min-w-125px text-center">{{ app()->getLocale() == 'en' ? 'Background Image' : 'Gambar Background' }}</th>
                                    <th class="min-w-100px text-center">{{ app()->getLocale() == 'en' ? 'Order' : 'Urutan' }}</th>
                                    <th class="min-w-100px text-center">{{ app()->getLocale() == 'en' ? 'Status' : 'Status' }}</th>
                                    <th class="pe-4 min-w-125px text-end rounded-end">{{ app()->getLocale() == 'en' ? 'Actions' : 'Aksi' }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                            ...
                                            @if($banner->image_url)
                                                <a href="javascript:void(0)" 
                                                   ...
                                                   title="{{ app()->getLocale() == 'en' ? 'Click to View Image Gallery' : 'Klik untuk Membuka Galeri Gambar' }}">
                                                    <img src="{{ asset($banner->image_url) }}" alt="Banner" style="width: 75px; height: 48px; object-fit: cover; display: block; pointer-events: none;" />
                                                </a>
                                            @else
                                                <span class="text-muted fs-7 italic">{{ app()->getLocale() == 'en' ? 'Default Hero BG' : 'BG Default' }}</span>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td class="text-center">
                                            <div class="form-check form-switch form-check-custom form-check-solid justify-content-center">
                                                <input class="form-check-input h-20px w-35px cursor-pointer" type="checkbox" 
                                                       {{ $banner->is_active ? 'checked' : '' }} onchange="toggleBannerStatus({{ $banner->id }})">
                                            </div>
                                        </td>
                                        <td class="pe-4 text-end">
                                            <span data-bs-toggle="tooltip" data-bs-placement="top" title="{{ app()->getLocale() == 'en' ? 'Edit Slide Banner' : 'Edit Slide Banner' }}">
                                                <button type="button" class="btn btn-icon btn-light-primary btn-sm me-1 shadow-xs" 
                                                        onclick="editBannerModal({{ json_encode($banner) }})">
                                                    <i class="ki-duotone ki-pencil fs-4"><span class="path1"></span><span class="path2"></span></i>
                                                </button>
                                            </span>

                                            <span data-bs-toggle="tooltip" data-bs-placement="top" title="{{ app()->getLocale() == 'en' ? 'Delete Slide Banner' : 'Hapus Slide Banner' }}">
                                                <button type="button" class="btn btn-icon btn-light-danger btn-sm shadow-xs" 
                                    ...
                                    <tr>
                                        <td colspan="7" class="text-center py-10">
                                            <div class="d-flex flex-column align-items-center">
                                                <i class="ki-duotone ki-picture fs-3x text-muted mb-3"><span class="path1"></span><span class="path2"></span></i>
                                                <span class="text-muted fs-6 fw-semibold mb-3">
                                                    {{ app()->getLocale() == 'en' ? 'No slide banners found.' : 'Belum ada slide banner beranda.' }}
                                                </span>
                                                <button type="button" class="btn btn-primary btn-sm" onclick="openAddBannerModal()">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content bg-transparent border-0 shadow-none">
                <div class="modal-header border-0 pb-2 justify-content-end">
                    <button type="button" class="btn btn-sm btn-icon btn-active-color-white bg-black bg-opacity-50 text-white rounded-circle" data-bs-dismiss="modal">
                    ...
                    <div id="bannerLightboxCarousel" class="carousel slide" data-bs-ride="false">
                        <div class="carousel-inner">
                            @foreach($banners as $i => $banner)
                                @if($banner->image_url)
                                    <div class="carousel-item {{ $i === 0 ? 'active' : '' }}" id="lightbox_slide_{{ $i }}">
                                        <div class="d-flex flex-column align-items-center">
                                            <img src="{{ asset($banner->image_url) }}" class="d-block img-fluid rounded shadow-lg" style="max-height: 75vh; object-fit: contain;" alt="Banner Slide {{ $i + 1 }}">
                                            <div class="mt-3 bg-black bg-opacity-75 text-white px-4 py-2 rounded-pill fs-6 fw-semibold">
                                                {{ $banner->title_prefix ? $banner->title_prefix . ' ' : '' }}{{ $banner->title_highlight }} ({{ $i + 1 }}/{{ $banners->count() }})
                                            </div>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <button class="carousel-control-prev" type="button" data-bs-target="#bannerLightboxCarousel" data-bs-slide="prev">
                            <span class="carousel-control-prev-icon bg-black bg-opacity-50 rounded-circle p-3" aria-hidden="true"></span>
                            <span class="visually-hidden">Previous</span>
                        </button>
                        <button class="carousel-control-next" type="button" data-bs-target="#bannerLightboxCarousel" data-bs-slide="next">
                            <span class="carousel-control-next-icon bg-black bg-opacity-50 rounded-circle p-3" aria-hidden="true"></span>
