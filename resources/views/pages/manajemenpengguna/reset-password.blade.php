...
@section('content')
            ...
            <div
                ...
                <div class="d-flex align-items-center gap-3">
                    <div class="symbol symbol-45px symbol-circle bg-light-primary p-2">
                        <i class="ki-duotone ki-lock-2 text-primary fs-2x"><span class="path1"></span><span
                                class="path2"></span><span class="path3"></span><span class="path4"></span><span
                                class="path5"></span></i>
                    </div>
                    <div>
                        <h2 class="text-gray-900 fw-bold fs-3 m-0">
                            {{ app()->getLocale() == 'en' ? 'Password Reset Requests Management' : 'Pengelolaan Permintaan Reset Password' }}
                        </h2>
                        <span class="text-muted fs-7">
                            {{ app()->getLocale() == 'en' ? 'Process password reset applications, generate new credentials, and reject invalid requests.' : 'Memproses pengajuan reset kata sandi pengguna, membuat kredensial baru, dan menolak pengajuan tidak valid.' }}
                        </span>
                    </div>
                </div>
                <div class="d-flex align-items-center gap-2 ms-auto">
                    <span data-bs-toggle="tooltip" data-bs-placement="top"
                        ...
                        <button type="button"
                            ...
                            data-bs-toggle="modal" data-bs-target="#kt_modal_reset_password_help">
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-4"><span class="path1"></span><span
                                    class="path2"></span></i>
                            <input type="text" id="reset_password_search_input"
                                class="form-control form-control-solid w-250px ps-12" placeholder="Cari nama / email..."
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="d-flex flex-column">
                                                    <span
                                                        class="text-gray-800 fw-bold mb-1">{{ $item->user->name ?? 'Pengguna' }}</span>
                                        ...
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span
                                                    class="text-gray-800 fw-bold">{{ $item->created_at ? $item->created_at->format('d M Y, H:i') : '-' }}</span>
                                                <span
                                                    class="fs-8 text-muted">{{ $item->created_at ? $item->created_at->diffForHumans() : '' }}</span>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            @if ($item->status == 'pending')
                                                <span class="badge badge-light-warning fw-bold px-3 py-2">
                                                    <i class="ki-duotone ki-timer fs-6 text-warning me-1"><span
                                                            class="path1"></span><span class="path2"></span><span
                                                            class="path3"></span></i> Pending
                                                </span>
                                            @elseif($item->status == 'completed')
                                                <span class="badge badge-light-success fw-bold px-3 py-2">
                                                    <i class="ki-duotone ki-check-circle fs-6 text-success me-1"><span
                                                            class="path1"></span><span class="path2"></span></i> Reset
                                                    Selesai
                                                </span>
                                            @else
                                                <span class="badge badge-light-danger fw-bold px-3 py-2">
                                                    <i class="ki-duotone ki-cross-circle fs-6 text-danger me-1"><span
                                                            class="path1"></span><span class="path2"></span></i> Ditolak
                                                </span>
                                            @endif
                                        </td>
                                        <td>
                                            @if ($item->handler)
                                                <div class="d-flex flex-column">
                                                    <span
                                                        class="fs-7 text-gray-800 fw-bold">{{ $item->handler->name }}</span>
                                                    <span
                                                        class="fs-8 text-muted">{{ $item->handled_at ? $item->handled_at->format('d M Y H:i') : '' }}</span>
                                                </div>
                                            @else
                                        ...
                                        <td class="text-end pe-4">
                                            @if ($item->status == 'pending')
                                                <button type="button" class="btn btn-sm btn-primary me-2 btn-do-reset"
                                                    data-id="{{ $item->id }}" data-email="{{ $item->email }}"
                                                    data-name="{{ $item->user->name ?? 'User' }}">
                                                    <i class="ki-duotone ki-key fs-4 me-1"><span
                                                            class="path1"></span><span class="path2"></span></i> Reset
                                                    Pass
                                                </button>
                                                <button type="button" class="btn btn-sm btn-light-danger btn-do-reject"
                                                    data-id="{{ $item->id }}" data-email="{{ $item->email }}">
                                                    Tolak
                                                </button>
                                            @else
                ...
                <form id="kt_modal_process_reset_form" action="#">
                    ...
                    <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <input type="text" name="password" id="reset_password_input"
                                class="form-control form-control-solid" value="Password!12345"
                                placeholder="Masukkan kata sandi baru (min. 8 karakter)" required />
                            <span class="text-muted fs-7">Default password: <code>Password!12345</code> (dapat diubah oleh
                                Admin jika diperlukan)</span>
                        </div>

                        <div class="fv-row mb-7">
                            <label class="required fw-semibold fs-6 mb-2">Konfirmasi Kata Sandi Baru</label>
                            <input type="text" name="password_confirmation" id="reset_password_confirm_input"
                                class="form-control form-control-solid" value="Password!12345"
                        ...
                        <div class="fv-row">
                            ...
                            <textarea name="admin_notes" class="form-control form-control-solid" rows="2"
                                placeholder="Catatan internal perihal reset password ini">Password otomatis di-reset ke Password!12345</textarea>
                        </div>
                    </div>
    ...
    @include('pages.manajemenpengguna.partials.reset-password-help-modal')
@endsection
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-do-reject', function() {
                ...
                Swal.fire({
                    ...
                    text: "Apakah Anda yakin ingin menolak permintaan reset password dari '" +
                        reqEmail + "'?",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ url('manajemenpengguna/reset-password') }}/" + reqId +
                                "/reject",
                            type: "POST",
                            data: {
                                _token: "{{ csrf_token() }}"
