...
@section('content')
            ...
            <div
                ...
                <div class="d-flex align-items-center gap-3">
                    <div class="symbol symbol-45px symbol-circle bg-light-primary p-2">
                        <i class="ki-duotone ki-profile-user text-primary fs-2x"><span class="path1"></span><span
                ...
                <div class="d-flex align-items-center gap-2 ms-auto">
                    <span data-bs-toggle="tooltip" data-bs-placement="top"
                        ...
                        <button type="button"
                            ...
                            data-bs-toggle="modal" data-bs-target="#kt_modal_akses_user_help">
                    ...
                    <div class="card-title">
                        <div class="d-flex align-items-center position-relative my-1">
                            <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-5"><span class="path1"></span><span
                                    class="path2"></span></i>
                            <input type="text" id="kt_akses_user_search"
                            ...
                            <tbody class="fw-semibold text-gray-600">
                                @foreach ($users as $user)
                                    <tr>
                                        <td class="d-flex align-items-center">
                                            ...
                                            <div class="d-flex flex-column">
                                                <a href="javascript:void(0)"
                                                    class="text-gray-800 text-hover-primary mb-1 fw-bold btn-manage-user-access"
                                                    data-id="{{ $user->id }}" data-bs-toggle="tooltip"
                                                    data-bs-placement="top"
                                                    title="Klik untuk kelola akses {{ $user->name }}">{{ $user->name }}</a>
                                        ...
                                        <td>
                                            ...
                                            @if ($allPerms->isNotEmpty())
                                                @php
                                                    ...
                                                    $permData = $groupedPerms
                                                        ->map(function ($permsInModule, $moduleName) use (
                                                            $directPermNames,
                                                        ) {
                                                            return [
                                                                'module' => $moduleName,
                                                                ...
                                                                'actions' => $permsInModule
                                                                    ->map(function ($p) use ($directPermNames) {
                                                                        return [
                                                                            'name' => $p->name,
                                                ...
                                                <div class="d-flex flex-column gap-1">
                                                    <div class="d-flex align-items-center gap-1 flex-wrap">
                                                        @if ($hasDirect)
                                                            <span
                                                                class="badge badge-light-warning fw-bold fs-8 cursor-pointer btn-show-user-permissions-drawer"
                                                                data-id="{{ $user->id }}"
                                                                data-name="{{ $user->name }}"
                                                                ...
                                                                <i class="ki-duotone ki-key fs-7 text-warning me-1"><span
                                                                        class="path1"></span><span
                                                                        class="path2"></span></i>
                                                                {{ $directPerms->count() }} Akses Langsung
                                                                ({{ $permCount }} Total Izin)
                                                            </span>
                                                        @else
                                                            <span
                                                                class="badge badge-light-success fw-bold fs-8 cursor-pointer btn-show-user-permissions-drawer"
                                                                data-id="{{ $user->id }}"
                                                                data-name="{{ $user->name }}"
                                                                data-email="{{ $user->email }}"
                                                                data-avatar="{{ $user->avatar_url }}"
                                                                ...
                                                                data-bs-toggle="tooltip" data-bs-placement="top"
                                                                title="Klik untuk membuka rincian {{ $permCount }} hak akses di panel samping (Side Drawer)">
                                                                <i class="ki-duotone ki-shield-tick fs-7 text-success me-1"><span
                                                                        class="path1"></span><span
                                                                        class="path2"></span></i>
                                                    ...
                                                    <div class="d-flex flex-wrap gap-1 align-items-center">
                                                        @foreach ($previewModules as $modName => $modPerms)
                                                            <span
                                                                class="badge badge-light-primary fs-8 text-truncate max-w-150px cursor-pointer btn-show-user-permissions-drawer"
                                                                data-id="{{ $user->id }}"
                                                                data-name="{{ $user->name }}"
                                                                data-email="{{ $user->email }}"
                                                                data-avatar="{{ $user->avatar_url }}"
                                                        ...
                                                        @if ($moduleCount > 2)
                                                            <span
                                                                class="badge badge-light-info fs-8 fw-bold cursor-pointer btn-show-user-permissions-drawer"
                                                                data-id="{{ $user->id }}"
                                                                data-name="{{ $user->name }}"
                                                                data-email="{{ $user->email }}"
                                                                data-avatar="{{ $user->avatar_url }}"
                                                                ...
                                                                <i class="ki-duotone ki-layout-4 fs-7 me-1"><span
                                                                        class="path1"></span><span
                                                                        class="path2"></span></i>
                                            ...
                                            @else
                                                <span class="badge badge-light-secondary text-gray-600"
                                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                                    title="Pengguna tidak memiliki hak akses atau role">
                                                    <i class="ki-duotone ki-cross-circle fs-6 text-gray-500 me-1"><span
                                        ...
                                        <td class="text-end">
                                            <button class="btn btn-sm btn-light-primary btn-manage-user-access"
                                                data-id="{{ $user->id }}">
                                                <i class="ki-duotone ki-key fs-4 me-1"><span class="path1"></span><span
                                                        class="path2"></span></i> Kelola Akses
                                            </button>
                                        </td>
                                    </tr>
                                @endforeach
    ...
    <div class="offcanvas offcanvas-end w-100 w-md-450px w-lg-500px shadow-lg border-0" tabindex="-1"
        id="kt_offcanvas_user_permissions">
        ...
        <div class="offcanvas-body p-6">
            ...
            <div
                class="d-flex align-items-center justify-content-between mb-5 p-4 rounded bg-light-warning border border-warning">
                <div class="d-flex align-items-center gap-3">
                ...
                <button type="button" class="btn btn-sm btn-warning fw-bold btn-manage-from-drawer"
                    id="btn_manage_from_drawer" data-id="">
            ...
            <div class="position-relative mb-5">
                <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-4 top-50 translate-middle-y text-gray-500"><span
                        class="path1"></span><span class="path2"></span></i>
                <input type="text" id="drawer_perm_search" class="form-control form-control-solid ps-12"
            ...
            var aksesUserTable = $('#kt_akses_user_table').DataTable({
                ...
                drawCallback: function() {
                    var tooltipTriggerList = [].slice.call(document.querySelectorAll(
                        '#kt_akses_user_table [data-bs-toggle="tooltip"]'));
            ...
            $(document).on('click', '.btn-show-user-permissions-drawer', function(e) {
                ...
                if (permsData && permsData.length > 0) {
                    permsData.forEach(function(group) {
                        ...
                        group.actions.forEach(function(act) {
                            ...
                            actionsHtml += '<span class="badge ' + badgeColor +
                                ' fs-8 px-2.5 py-1 me-1 mb-1" title="' + badgeTitle + '">' +
                                act.action + (act.is_direct ? ' *' : '') + '</span>';
                        });

                        html +=
                            '<div class="card schema-card border border-gray-300 p-4 rounded module-perm-card" data-search="' +
                    ...
                    if (directPermCount > 0) {
                        $('#drawer_perm_summary').text(directPermCount + ' Akses Langsung & ' + totalPerms +
                            ' Total Izin di ' + totalModules + ' Modul');
                    } else {
                        $('#drawer_perm_summary').text(totalPerms +
                            ' Total Izin (Seluruhnya Diwarisi dari Role)');
                    }
                } else {
                    html =
                        '<div class="text-center py-8 text-muted"><i class="ki-duotone ki-information-2 fs-2x mb-2 text-muted"><span class="path1"></span><span class="path2"></span><span class="path3"></span></i><div>Tidak ada hak akses.</div></div>';
